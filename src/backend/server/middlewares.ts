...
// `conf.Conf.JwtSecret` 一次性确定；TS 版必须按 env 隔离才能保持同样的语义。
...
export async function getJwtSecret(c?: Context | any): Promise<string> {
  ...
  // 性能：命中缓存后直接返回，避免每次调用都回源存储。修复前此分支只
  // `return kvSecret` 而不写缓存，导致 getUserFromContext / csrfProtection /
  // checkAdminAuth 等每请求 2-4 次调用都会各自触发一次后端读取（KV/D1 后端下
  // 这是最贵的操作之一）。
  ...
  // 约定 JWT 与字段加密共用同一把密钥，因此这里显式回退读取加密密钥槽位，
  // 保证「生成了一份」就等于「两边都可用」。
  try {
