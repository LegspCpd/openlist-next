...
| 変数名 | 説明 | 設定必須か |
|---|---|---|
| `EO_KV_URLS` | EdgeOne 専用。EdgeOne の KV バインディングはエッジ関数にしか渡されず、Node クラウド関数からは取得できないため、同じデプロイのエッジ関数を経由してプロキシする必要があります。ここにはそのプロキシアドレスを設定します | 通常は設定不要。空のままなら現在のドメインが自動使用されます。クロスオリジンやローカルデバッグの場合にのみ必要 |
